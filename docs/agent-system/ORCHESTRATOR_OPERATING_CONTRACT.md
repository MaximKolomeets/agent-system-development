...
Для write-action/substantive Engine-блоков оркестратор добавляет fenced YAML `task_contract` по `docs/agent-system/TASK_CONTRACT.md`. Contract является source of truth для mode/scope/checks/STOP, prose остаётся human explanation. Fast Lane без write-action, PR и journal trace может идти без `task_contract`. Для substantive task prose также включает `AUTONOMOUS_TERMINAL_EXECUTION_PROTOCOL.md`: два terminal outcome, adaptive scope envelope и отдельные iteration budgets.
...
Перед target adoption или source-update определить Variant A/B/C или STOP по `docs/agent-system/TARGET_ADOPTION_DETECTOR.md`; dirty target tree, unstable methodology source или риск overwrite target-specific journal/history/state означает STOP.
Перед file-changing task/PR применять `docs/agent-system/QUALITY_FIRST_WORKFLOW.md`: task prompt должен иметь Definition of Ready, проверяемые acceptance criteria, self-review before PR, PR body quality check и blocker-ID based fix-pass policy. Failed отдельный check не объявлять STOP без классификации по `AUTONOMOUS_TERMINAL_EXECUTION_PROTOCOL.md`: recoverable scoped failure исправляется в той же task branch, а STOP допустим только по его closed taxonomy.
...
Для file-changing задачи требуй terminal execution по `docs/agent-system/AUTONOMOUS_TERMINAL_EXECUTION_PROTOCOL.md` и `docs/agent-system/EXECUTION_CONTINUATION_POLICY.md`: engine доводит recoverable scoped работу до `ready_for_human_review` либо документированного `stopped_human_required`, фиксируя dependency closure и budgets.
Первый шаг исполнителя (engine) в substantive task — зафиксировать `execution_started_at` в ISO 8601 с timezone до содержательных чтений, правок, проверок и repository-mutating действий; это значение сохраняется в TASK и не перезаписывается при финализации RESULT. В конце task фиксируются `execution_finished_at` и вычисленный `execution_duration`.
...
6. В substantive Engine-блоках требовать первый measured шаг `execution_started_at` до работы и перенос этого значения в RESULT без перезаписи.
7. Не расширять methodology repository после adoption без blocker или отдельного решения пользователя.
