...
4a. Для file-changing задач применять `AUTONOMOUS_TERMINAL_EXECUTION_PROTOCOL.md` вместе с `EXECUTION_CONTINUATION_POLICY.md`: engine исправляет recoverable scoped findings, использует заранее объявленные budgets и завершает PR/CI либо документирует единственный настоящий `stopped_human_required`.
