...
## 2026-06-06 - CORP_KNOWLEDGE_PLATFORM remains private

Решение:
Публичность `agent-system-development` не переносится автоматически на `CORP_KNOWLEDGE_PLATFORM`.

Причина:
`CORP_KNOWLEDGE_PLATFORM` может содержать внутреннюю архитектуру, корпоративные процессы, интеграции, потенциально чувствительные данные и будущие клиентские материалы.

Последствия:

- `CORP_KNOWLEDGE_PLATFORM` должен оставаться private;
- для полноценной защиты private repository может потребоваться GitHub Pro/Team/Organization;
- до этого используется ручной workflow и review.
