...
`WORKFLOW.md`, `HOTFIX_AND_ROLLBACK_POLICY.md`, `DISASTER_RECOVERY.md`,
`RELEASE_READINESS.md` и `NEXT_STEPS.md`.
...
| Rollback/hotfix decision | Agent может собрать факты и подготовить rollback PR/commands по `HOTFIX_AND_ROLLBACK_POLICY.md` | Только человек-архитектор принимает решение о rollback/hotfix и merge в `main` | Decision evidence, target commit/tag, bad SHA, PR/incident link без private data. |
...
1. Проверить `HOTFIX_AND_ROLLBACK_POLICY.md`: incident scope, branch
   `work/hotfix/<issue>`, bad SHA, target tag/commit и review/check evidence.
2. Проверить `DISASTER_RECOVERY.md`, если GitHub, local checkout или tokens
   недоступны.
3. Убедиться, что merge в `main`, tag/publication/sync и final rollback
   decision выполнены человеком или ожидают human gate.
