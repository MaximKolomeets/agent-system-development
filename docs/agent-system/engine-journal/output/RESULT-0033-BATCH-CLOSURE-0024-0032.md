...
- PR URL: `to be stamped after PR creation`.
- PR number: `to be stamped after PR creation`.
...
В старых RESULT сохранены исходные pre-merge строки вроде `ready for review`,
`open; not merged`, `PR open` и технический literal `see Engine final report`,
...
- `git diff --name-only developer...HEAD` — pending before commit.
- `git rev-parse --abbrev-ref HEAD` — pending before commit.
- stale-status scan по closure-set и `INDEX.md` — pending before commit.
