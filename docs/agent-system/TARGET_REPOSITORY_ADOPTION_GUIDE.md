...
1. Зафиксировать target repository, base branch, working branch и selected branch model.
2. Прочитать stable methodology reference без `git switch/pull/checkout/reset/clean/stash` в рабочем methodology repository.
3. Добавить в TASK/RESULT/adoption artifact блок `methodology_reference` с `ref: origin/main`, `stable_only: true`, `source_commit` и `checked_at`.
4. Перенести Russian-first policy в target `AGENTS.md` или эквивалентные target instructions, если scope меняет такие инструкции.
5. Материализовать только target-adapted docs: project state, branch names, governance pack и journal history не копируются verbatim из methodology repository.
