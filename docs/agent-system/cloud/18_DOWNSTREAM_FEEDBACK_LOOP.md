...
- suggested backlog grouping или task boundary.
...
## MIR lifecycle ledger

После sanitization и classification reusable item может попасть в
`METHODOLOGY_IMPROVEMENT_LEDGER.md`.

Ledger фиксирует только public-safe lifecycle:

```text
feedback -> MIR -> PR -> release -> adoption
```

Consumer-level rollout state, real repository names, private PRs и blockers
живут только в private control plane по templates:
...
- `main`;
- release tag;
- explicitly published Source/cloud snapshot, опубликованный как stable source.
...
- "Downstream dry-run показал repeated mismatch между acceptance scenario,
  blocker code и fixture expectation."
- "Feedback category: real-data boundary должен быть explicit before protocol
  generation."
- "Feedback category: task scope должен distinguish docs-only, fixture-only и
  implementation slices."

Unsafe:

- "В project X file Y line Z had blocker code ABC missing."
- "Этот customer protocol used real device serial number..."
- "Repository X branch Y имел uncommitted tests..."
