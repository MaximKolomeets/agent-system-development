...
4. **Branch policy setup** — `main` (стабильная), `developer` (интеграционная), `work/<role>/*` (рабочие); после bootstrap прямые изменения в `main`/`developer` запрещены процессом или rulesets/branch protection.
5. **Role/worktree setup** — роли по ответственности (без vendor/tool names), исполнитель отдельно как `engine`; каждая задача — в `work/<role>/<task>`.
...
Для нового repository со стандартной схемой `main -> developer -> work/<role>/*` ветка `developer` должна быть создана до первой рабочей задачи `engine`.
...
- только потом создавать `work/<role>/*` от `developer`.
