...
Текущий фокус: `METH-FIX-AUTHORIZATION-HEADER-GUARD-01` закрывает P1 safety regression в ready-gate, найденный во время adoption PR #52 в target implementation repository. Cleanup PR #267 уже смержен в `developer`; `METH-STABLE-MAIN-REFERENCE-RUSSIAN-FIRST-01` и `METH-NO-ORDINARY-POST-MERGE-CLOSURE-01` уже попали в `developer`; release `v1.2.0` выпущен через PR #253 и tag `v1.2.0`, sync `main -> developer` выполнен через PR #254, последующий post-release advance `main` синхронизирован через PR #259. F-03 остаётся pending human-action: тег на текущий `main` не ставился; если архитектор считает payload PR #258 отдельным release, annotated tag ставит только человек. После merge hotfix архитектор может выпустить patch source и обновить affected target implementation repository PR от нового stable source. Точные task/PR факты брать из `engine-journal/INDEX.md` и `RESULT-*` closure/release stamps.
