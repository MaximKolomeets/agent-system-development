...
```text
main -> developer -> work/<role>/<task> -> PR в developer

Внутренние `work/<role>/<task>/*` допустимы только как sub-branches внутри той же substantive task и не являются отдельными delivery PR.
```
...
Findings не исправляются внутри standalone review-only task.
...
Пользователь принимает отдельное решение, какие findings из standalone review-only task превращать в implementation PR.
