...
Reviewer roles не исправляют найденные проблемы по умолчанию. Они возвращают **тело** review report в чат по умолчанию; одновременно **всегда** журналируют TASK + RESULT и обновляют INDEX по `docs/agent-system/ENGINE_JOURNAL_CONTRACT.md` и создают docs-only PR с journal artefacts в `developer`.
...
`Report delivery: chat` **не** отменяет journal trace. «Chat-only» относится к телу отчёта, а не к отсутствию journal-следа.
