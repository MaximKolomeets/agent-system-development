...
| `INV-JOURNAL-SEQUENCE-RESERVATION` | Новое выделение sequence учитывает merged INDEX, append-only reservation ledger и versioned provider-neutral snapshot; duplicate claim fail-closed, abandoned tombstone не переиспользуется. | `JOURNAL_SEQUENCE_RESERVATION.md`, `SEQUENCE_RESERVATIONS.json`, `validate_journal_sequence_reservations.py` | Локальная structural validation и CI strict provider snapshot блокируют collision до merge. |
