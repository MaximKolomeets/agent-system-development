...
```text
Архитектору — загрузить в контекст оркестратора: <изменённые source/template файлы этой задачи> + PROJECT_FILE_MAP.md (если менялся inventory) [+ INDEX/state если менялись]. Обновлённые bundle-файлы скопированы в `docs/agent-system/cloud/` (regen) — брать оттуда. asof: <ISO-8601 timestamp>; developer_head_sha: <sha>.
```
...
- добавлять пометку про `docs/agent-system/cloud/`, если был изменён bundle-файл или cloud bundle был регенерирован;
