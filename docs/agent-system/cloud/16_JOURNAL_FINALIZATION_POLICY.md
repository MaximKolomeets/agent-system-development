...
## Канонический pre-merge verdict reviewer gate

Единственная строка с `pending`, которая является финализированным verdict, а
не placeholder:
...
Она допустима только целиком, только в `TASK-*` или `RESULT-*` текущей
контрактной записи. Значение означает, что технический reviewer gate прошёл,
а merge остаётся полномочием человека; оно не обещает дополнить RESULT,
checks, PR URL или SHA позднее.

Любая другая строка с этим token блокируется: произвольная фраза, другой
контекст, неизвестное значение поля, изменённое значение или добавленное
обещание будущей финализации. Глобальное исключение слова `pending` не
