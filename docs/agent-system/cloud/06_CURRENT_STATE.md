...
Текущий фокус: `METH-RELEASE-GATE-READY-MODE-01` устраняет STOP перед release candidate `v1.4.0`: попытка release PR `developer -> main` была остановлена до PR creation, потому что обычный `check_task_ready.py --base origin/main` правомерно применил work-branch-only blockers к `developer` и нашёл deferred-marker wording в `RESULT-0116`. Задача добавляет явный release-boundary mode для ready-gate и исправляет finalized wording `RESULT-0116`; release PR, merge, tag, target repositories, runtime, Docker, CI и branch protection не выполнялись. Следующий шаг после merge этой задачи: повторить `METH-RELEASE-V1-4-0-01`.
