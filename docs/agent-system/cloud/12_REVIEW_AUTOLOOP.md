...
Если feedback поступает как несколько внешних human review rounds по одному
документу/решению и уже не привязан к одному active work PR, использовать
`docs/agent-system/EXTERNAL_REVIEW_LEDGER_PATTERN.md`: ledger фиксирует
`applied`/`deferred`/`rejected`, anti-loop правило и diminishing-returns stop.
Autoloop не заменяет такой ledger и не должен превращаться в бесконечную
полировку вне active PR.
...
Для external human review rounds вне active PR аналогичный stop rule задает
`EXTERNAL_REVIEW_LEDGER_PATTERN.md`: повторяющиеся или non-critical remarks
записываются в ledger и не запускают новый blocking cycle.
