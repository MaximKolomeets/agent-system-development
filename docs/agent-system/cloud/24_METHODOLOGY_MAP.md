...
| Release, state refresh или boundary reconciliation | `RELEASE_READINESS.md`, `CURRENT_STATE.md`, `NEXT_STEPS.md`, `ENGINE_JOURNAL_CONTRACT.md` | Ordinary PR lifecycle не расширяется в per-PR closure; release/audit boundary сверяется отдельно. |
...
доступов, publication boundary, security/privacy risk model или комментариев в
технических файлах.
...
`templates/THREAT_MODEL_TEMPLATE.md`; language/commenting style ->
