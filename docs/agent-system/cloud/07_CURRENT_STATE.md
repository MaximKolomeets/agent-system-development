...
- Terminal execution закреплён для file-changing задач: engine доводит scoped работу до `ready_for_human_review` с evidence либо до документированного `stopped_human_required`; recoverable checks/generation/review failures исправляются в той же task branch. `AUTONOMOUS_TERMINAL_EXECUTION_PROTOCOL.md` задаёт closed STOP taxonomy, decision fallback, adaptive scope envelope и budgets, а `EXECUTION_CONTINUATION_POLICY.md` сохраняет continuation/dirty-scope safeguards.
...
PR #364 выделил sequence 0174, а Human UAT evidence PR #365 merged в
`developer` at `2026-08-04T07:45:40Z`. Sequence 0174 закрыта, reservation
consumed. Это journal/state delta, не новый release payload.
...
шаг полного final release pass выполняется sequence 0178 в PR #381: independent reviewer consistency-gate проверяет immutable candidate `9a23a8efebc9c41df13843a543afb73bd6bd6392`. Только после её human merge готовится release PR `developer -> main`; отдельный repository-changing closure этой evidence-only gate до release PR не создаётся.
...
шаг полного final release pass — завершить reviewer consistency-gate sequence 0178 / PR #381 на immutable candidate; immutable gate 0175 не переиспользуется. После human merge PR #381 выполняется release policy gate без нового repository-changing closure. Release merge, tag
