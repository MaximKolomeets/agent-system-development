...
Перед file-changing task/PR применять `docs/agent-system/QUALITY_FIRST_WORKFLOW.md`: task prompt должен иметь Definition of Ready, проверяемые acceptance criteria, self-review before PR, PR body quality check и blocker-ID based fix-pass policy.
...
Для file-changing задачи требуй terminal execution по `docs/agent-system/EXECUTION_CONTINUATION_POLICY.md`: engine доводит scoped работу до `ready_for_human_review` либо документированного `stopped_human_required`.
