...
Для file-changing задач обязательна ссылка на `docs/agent-system/QUALITY_FIRST_WORKFLOW.md`: Definition of Ready, проверяемые acceptance criteria, self-review before PR, PR body quality check и blocker-ID based fix-pass. Missing acceptance criteria означает STOP, кроме простых консультационных задач без write-action.
...
```text
Следующий: архитектор — merge; затем engine — синхронизировать developer и перейти к следующей задаче или следующему шагу серии; отдельный journal closure PR не создавать; boundary reconciliation — только перед release/audit boundary или по явному запросу архитектора.
```

Для финального PR серии:

```text
Следующий: архитектор — merge; затем engine — boundary reconciliation journal <seq-range>, если это release/audit boundary или явный запрос архитектора; затем release dev→main.
```

## Closure-status policy

Для ordinary PR отсутствие merge commit SHA / `merged_at` в `RESULT` после human merge не является blocker. GitHub PR metadata является source of truth для merge facts. Не создавать отдельный closure PR после каждого merge.

Boundary reconciliation нужна только перед release boundary, audit boundary, при explicit architect request или для batch reconciliation. В этих контекстах stale `RESULT/INDEX` внутри заданного scope остаются blocker.

Journal closed для release/reviewer gate означает: entries, включённые в boundary scope, закрыты closure-stamp/status+PR URL, а lifecycle-only entries со статусом `terminal-fold accepted` допустимы. STOP при `open`, `ready`, `closure pending`, `stamp at merge` или merged-but-unclosed entry только внутри boundary reconciliation scope; не STOP при ordinary terminal state или `terminal-fold accepted`. Если terminal fold не lifecycle-only или содержит незакрытый содержательный payload, STOP и запросить reconciliation/fix.
