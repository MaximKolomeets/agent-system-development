...
Default-режим закрытия journal после merge обычного рабочего PR — **batch**, а не отдельный post-merge closure PR.

До merge основная task entry может иметь статус `ready_for_merge` / `ready for review`, если `RESULT` и `INDEX` финализированы после PR creation и не содержат unresolved placeholders. После merge обычного work PR journal entry может временно оставаться в состоянии `open; not merged`, `PR open`, `not merged`, `ready for review` или `merged; closure pending`, если задача явно работает в batch-closure policy и серия изменений ещё продолжается. GitHub PR metadata является авторитетом для merge facts до batch-closure. Это допустимое промежуточное состояние, а не final state.
...
- release boundary, audit boundary, methodology version boundary или крупный milestone;
...
- пользователь явно выдал closure-задание;
- journal/RESULT содержит противоречивые или опасно устаревшие факты;
- создается или закрывается release PR `developer -> main`.
...
- если это обычный work PR, status `ready_for_merge` допустим до merge, а post-merge closure планируется batch-проходом перед boundary;
