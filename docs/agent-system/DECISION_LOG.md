...
Причина:
После PR-2m и PR-2n нужно проверить release candidate, forbidden paths, sensitive/private markers, актуальность state docs и readiness checklist без неявного выполнения release.

Последствия:

- PR-2o документирует release candidate `developer` -> `main`;
