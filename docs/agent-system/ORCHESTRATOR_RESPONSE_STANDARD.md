...
Если `RESULT` или `INDEX` остаются в pre-merge state после merge/release/sync, оркестратор должен дать полный self-contained блок для исполнителя (engine) на docs-only journal closure cleanup.
