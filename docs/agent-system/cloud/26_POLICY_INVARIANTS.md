...
| `INV-JOURNAL-TRIPLET` | Каждая новая journal entry — exact `TASK -> RATIONALE -> RESULT` с одним четырёхзначным sequence и task id; legacy rows используют `legacy/not_required`. RATIONALE содержит краткое проверяемое обоснование, а не hidden chain-of-thought, и остаётся append-only. | `ENGINE_JOURNAL_CONTRACT.md`, `TASK_CONTRACT.md`, `validate_journal_triplet.py`, `check_journal_append_only.py` | Triplet validator, ready-gate и CI блокируют missing/mismatched RATIONALE и unsafe rewrite. |
| `INV-JOURNAL-SEQUENCE-RESERVATION` | Новое выделение sequence учитывает merged INDEX, append-only reservation ledger и versioned provider-neutral snapshot; active reservation имеет ровно один strict PR/MR claim, adapter получает все страницы, ledger сохраняет base history prefix, а abandoned tombstone не переиспользуется. Provider CI передаёт credential только через environment с минимальными read-only permissions; отсутствие credential и любая unavailable причина остаются blocker без раскрытия sensitive diagnostics. | `JOURNAL_SEQUENCE_RESERVATION.md`, `SEQUENCE_RESERVATIONS.json`, `validate_journal_sequence_reservations.py` | Локальная structural/base validation и CI strict provider snapshot блокируют collision, partial scan, отсутствующий credential и rewrite ledger до merge. |
