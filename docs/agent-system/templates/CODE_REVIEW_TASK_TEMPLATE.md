...
Reviewer применяет `docs/agent-system/QUALITY_FIRST_WORKFLOW.md`: проверяет, что PR имел acceptance criteria, self-review before PR, чистый PR body и fix-pass policy по blocker IDs; reviewer focus остаётся scoped, без повторного full methodology audit без отдельного scope.
